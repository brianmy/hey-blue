...
function DiamondBase({
  ...
  return (
    <button
      ...
      className={`w-9 h-9 rotate-45 rounded-md border-2 transition-all duration-150 ${
        occupied
...
function GameStatePanel({ gs, onChange }: { gs: GameState; onChange: (gs: GameState) => void }) {
  const segBtn = (active: boolean, color: string) =>
    `w-8 h-8 rounded-lg text-sm font-bold transition-all duration-150 border ${
      active ? `${color} text-white` : 'bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-500'
    }`

  return (
    <div className="rounded-2xl border border-slate-700/60 bg-slate-900/60 backdrop-blur-sm p-3 space-y-2.5">

      {/* Row 1: Inning + Outs */}
      <div className="flex items-end gap-4">
        <div>
          <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Inning</p>
          <div className="flex items-center gap-1">
            <div className="flex rounded-lg overflow-hidden border border-slate-700">
              {(['top', 'bottom'] as const).map((h) => (
                <button key={h} type="button" onClick={() => onChange({ ...gs, half: h })}
                  className={`px-2 py-1 text-xs font-semibold transition-colors ${
                    gs.half === h ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'
                  }`}>
                  {h === 'top' ? '▲' : '▼'}
                </button>
              ))}
            </div>
            <button type="button" onClick={() => onChange({ ...gs, inning: Math.max(1, gs.inning - 1) })}
              className="w-6 h-6 rounded-md bg-slate-800 border border-slate-700 text-slate-300 font-bold hover:bg-slate-700 transition-colors text-xs">−</button>
            <span className="text-white font-bold text-sm w-4 text-center">{gs.inning}</span>
            <button type="button" onClick={() => onChange({ ...gs, inning: Math.min(12, gs.inning + 1) })}
              className="w-6 h-6 rounded-md bg-slate-800 border border-slate-700 text-slate-300 font-bold hover:bg-slate-700 transition-colors text-xs">+</button>
          </div>
        </div>
        <div>
          <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Outs</p>
          <div className="flex gap-1">
            {[0, 1, 2].map((n) => (
              <button key={n} type="button" onClick={() => onChange({ ...gs, outs: n })}
                className={segBtn(gs.outs === n, 'bg-red-500 border-red-400 shadow-md shadow-red-500/30')}>
                {n}
              </button>
            ))}
          </div>
      ...
      {/* Row 2: Count (left) + Diamond (right) */}
      <div className="flex items-start gap-3">
        {/* Balls + Strikes stacked */}
        <div className="space-y-1.5">
          <div>
            <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Count</p>
            <div className="flex items-center gap-1">
              <span className="text-[10px] text-green-400 font-bold w-3">B</span>
              {[0, 1, 2, 3].map((n) => (
                <button key={n} type="button" onClick={() => onChange({ ...gs, balls: n })}
                  className={segBtn(gs.balls === n, 'bg-green-500 border-green-400 shadow-md shadow-green-500/20')}>
                  {n}
                </button>
              ))}
            </div>
          ...
          <div className="flex items-center gap-1">
            <span className="text-[10px] text-yellow-400 font-bold w-3">S</span>
            {[0, 1, 2].map((n) => (
              <button key={n} type="button" onClick={() => onChange({ ...gs, strikes: n })}
                className={segBtn(gs.strikes === n, 'bg-yellow-500 border-yellow-400 shadow-md shadow-yellow-500/20')}>
                {n}
              </button>
            ))}
          </div>
        ...
        <div className="ml-auto flex flex-col items-center gap-0.5">
          <p className="text-[10px] text-slate-500 uppercase tracking-wider">Runners</p>
          <div className="relative w-24 h-24">
            <div className="absolute top-0 left-1/2 -translate-x-1/2">
              <DiamondBase label="2B" occupied={gs.runners.includes('second')}
                onClick={() => onChange({ ...gs, runners: toggleRunner(gs.runners, 'second') })} />
            </div>
            <div className="absolute top-1/2 left-0 -translate-y-1/2">
              <DiamondBase label="3B" occupied={gs.runners.includes('third')}
                onClick={() => onChange({ ...gs, runners: toggleRunner(gs.runners, 'third') })} />
            </div>
            <div className="absolute top-1/2 right-0 -translate-y-1/2">
              <DiamondBase label="1B" occupied={gs.runners.includes('first')}
                onClick={() => onChange({ ...gs, runners: toggleRunner(gs.runners, 'first') })} />
            </div>
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2">
              <div className="w-9 h-9 rotate-45 rounded-md border-2 border-slate-700 bg-slate-800/50 flex items-center justify-center">
      ...
      <div className="border-t border-slate-800 pt-1.5">
        <p className="text-xs text-slate-500 leading-relaxed">{buildGameStateDescription(gs)}</p>
      </div>
    </div>
  )
}
...
export default function Home() {
  ...
  const { fieldMode, toggle: toggleTheme } = useTheme()
      ...
      <header className="relative overflow-hidden border-b border-slate-800">
        ...
        <div className="relative px-5 py-5 flex items-center justify-between max-w-lg mx-auto w-full">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={toggleTheme}
              title={fieldMode ? 'Switch to dark mode' : 'Switch to field mode'}
              className="flex items-center gap-1 text-xs font-semibold text-slate-400 hover:text-blue-400 border border-slate-700 hover:border-blue-500 px-2.5 py-1.5 rounded-lg transition-colors"
            >
              {fieldMode ? '🌙 Dark' : '☀ Field'}
            </button>
          </div>
        </div>
      </header>

      <div className="flex-1 max-w-lg mx-auto w-full px-4 py-3 space-y-3">
        ...
        {result && (
          <div className="rounded-2xl border border-slate-700/60 overflow-hidden shadow-xl">
            ...
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 px-5 py-5">
              ...
              <p className="ruling-text text-2xl font-bold text-white leading-snug">{result.ruling}</p>
            </div>
